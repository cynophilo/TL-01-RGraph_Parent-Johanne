...
RGraph.SemiCircularProgress = function (conf) {
  ...
  this.draw = function () {
    ...
    if (typeof properties.centerx === "number")
      this.centerx = properties.centerx;
    if (typeof properties.centery === "number")
    ...
    if (RGraph.isString(properties.centerx))
      this.centerx += Number(properties.centerx);
    if (RGraph.isString(properties.centery))
      this.centery += Number(properties.centery);
    if (RGraph.isString(properties.radius))
  ...
  this.drawMeter = function () {
    ...
    this.path(
      ...
      properties.shadow ? properties.shadowColor : "rgba(0,0,0,0)",
    ...
    if (RGraph.isNumber(this.value)) {
      var angle =
        (end - start) *
        ((this.value - this.scale2.min) / (this.max - this.scale2.min));
      ...
      for (var i = 0; i < this.value.length; ++i) {
        var angle =
          (properties.anglesEnd - properties.anglesStart) *
          ((this.value[i] - this.scale2.min) / (this.max - this.scale2.min));
  ...
  this.drawLabels = function () {
    ...
    if (properties.labelsMin) {
      ...
      if (!RGraph.isNull(properties.labelsMinSpecific)) {
        ...
        var text = RGraph.numberFormat({
          object: this,
          number: this.scale2.min.toFixed(
            typeof properties.labelsMinDecimals === "number"
              ? properties.labelsMinDecimals
    ...
    if (properties.labelsMax) {
      ...
      if (!RGraph.isNull(properties.labelsMaxSpecific)) {
        ...
        var text = RGraph.numberFormat({
          object: this,
          number: this.scale2.max.toFixed(
            typeof properties.labelsMaxDecimals === "number"
              ? properties.labelsMaxDecimals
              : properties.scaleDecimals
          ),
    ...
    if (properties.labelsCenter) {
      ...
      if (!RGraph.isNull(properties.labelsCenterSpecific)) {
        ...
        var text = RGraph.numberFormat({
          object: this,
          number: RGraph.isNumber(this.value)
            ? this.value.toFixed(
  ...
  this.drawBackgroundGrid = function () {
    if (properties.backgroundGrid) {
      ...
      if (properties.backgroundGridRadials) {
        ...
        var increment =
          (properties.anglesEnd - properties.anglesStart) /
          properties.backgroundGridRadialsCount;
    ...
    if (properties.scale) {
      ...
      for (var i = 0; i < this.scale2.labels.length; ++i) {
        ...
        var xy = RGraph.getRadiusEndPoint({
          ...
          angle:
            properties.anglesStart +
            ((i + 1) / this.scale2.labels.length) *
              (properties.anglesEnd - properties.anglesStart),
          radius:
            this.radius +
            (properties.backgroundGrid ? properties.backgroundGridMargin : 0) +
            textConf.size +
            properties.scaleLabelsOffsetr +
            5,
        });
      ...
      RGraph.text({
        ...
        halign: "center",
        text:
          typeof properties.scaleFormatter === "function"
            ...
            : (properties.scaleUnitsPre || "") +
              properties.scaleMin
                .toFixed(properties.scaleDecimals)
                .replace(/\./, properties.scalePoint) +
  ...
  this.getValue = function (e) {
    var mouseXY = RGraph.getMouseXY(e),
      mouseX = mouseXY[0],
      mouseY = mouseXY[1],
      angle = RGraph.getAngleByXY(this.centerx, this.centery, mouseX, mouseY);

    if (angle && mouseX >= this.centerx && mouseY > this.centery) {
      angle += RGraph.TWOPI;
    }

    if (angle < properties.anglesStart && mouseX > this.centerx) {
      angle = properties.anglesEnd;
    }
    if (angle < properties.anglesStart) {
      angle = properties.anglesStart;
    }
  ...
  this.getAngle = function (value) {
    ...
    var angle =
      (value / this.max) * (properties.anglesEnd - properties.anglesStart);
    angle += properties.anglesStart;
  ...
  this.overChartArea = function (e) {
    ...
    this.path(
      ...
      this.coords[0][0],
      this.coords[0][1],
      this.coords[0][2] - this.coords[0][5],
      properties.anglesEnd,
      properties.anglesStart
    );
  ...
  this.parseColors = function () {
    ...
    if (this.original_colors.length === 0) {
      this.original_colors.backgroundColor = RGraph.arrayClone(
        properties.backgroundColor
      );
      this.original_colors.colors = RGraph.arrayClone(properties.colors);
    }

    properties.colors[0] = this.parseSingleColorForGradient(
      properties.colors[0]
    );
    properties.colors[1] = this.parseSingleColorForGradient(
      properties.colors[1]
    );

    properties.colorsStroke = this.parseSingleColorForGradient(
      properties.colorsStroke
    );
  ...
  this.grow = function () {
    ...
    if (RGraph.isNumber(this.value)) {
      ...
      for (var i = 0; i < this.value.length; ++i) {
        initial_value[i] = RGraph.isNull(this.currentValue)
          ? 0
          : this.currentValue[i];
        diff[i] =
          this.value[i] -
          Number(RGraph.isNull(this.currentValue) ? 0 : this.currentValue[i]);
  ...
  this.tooltipsFormattedCustom = function (specific, index) {
    var color =
      ...
      properties.tooltipsFormattedKeyColors[specific.index]
        ? properties.tooltipsFormattedKeyColors[specific.index]
        : properties.colors[specific.index];
    var label =
      properties.tooltipsFormattedKeyLabels &&
